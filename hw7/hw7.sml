...
datatype geom_exp = 
	 ...
	 | Var of string
	 | Shift of real * real * geom_exp (* x, y, exp *)                                                              
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift(dx,dy,e1) => (* The SML code problem 2 *)
	  		case eval_prog(e1, env) of
				NoPoints => NoPoints
			  | Point(x,y) => Point(x + dx, y + dy)
			  | Line(m,b) => Line(m, b + dy - m * dx) (* b + deltaY − m · deltaX *)
	...
	case e of 
		LineSegment(x1,y1,x2,y2) =>
			...
			then 
				if real_close(y1, y2)
				then 
					Point(x1, y1) (* end points are the same *)
				else 
					if y2<y1
					then LineSegment(x2, y2, x1, y1) (* reorder *)
					else e
			else 
				if x2 < x1
				then LineSegment(x2, y2, x1, y1) (* reorder *)
	  ...
	  | _ => e (* others that dont have exp, simply return *)